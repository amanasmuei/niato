// Thrown by `niato.run()` when an input validator rejects the user
...
export class NiatoInputRejectedError extends Error {
  readonly reason: string;
  constructor(reason: string) {
    super(`Input rejected: ${reason}`);
    this.name = "NiatoInputRejectedError";
    this.reason = reason;
  }
}

// Thrown by `niato.run()` at turn start when a session's cumulative cost
...
export class NiatoBudgetExceededError extends Error {
  ...
  constructor(cumulativeUsd: number, limitUsd: number) {
    ...
    this.name = "NiatoBudgetExceededError";
