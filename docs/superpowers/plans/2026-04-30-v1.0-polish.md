...
**Goal:** Ship Niato v1.0.0 — the polish gate before public release. Two focused changes (better error UX, sensible default packs) plus the v1.0 release ceremony. Deliberately scopes out items the roadmap flagged as conditional (eval baselines need API budget; `pr_creator` needs real GitHub MCP wiring) — those move to a v1.x backlog rather than blocking v1.0.

**Architecture:** Two pure additions:
1. `src/cli/error-classify.ts` — pattern-matches an `unknown` thrown error against the common SDK failure shapes (network, 401, 429, malformed-classifier) and returns a friendly Ink-renderable explanation. The TUI session hook (`src/cli/tui/hooks/use-niato-session.ts:118-130`) calls it; the existing `phase: "error"` rendering already exists, so no new screens.
2. TUI's `niatoFactory` (`src/cli/tui/index.tsx:39`) loads only `genericPack` by default. `NIATO_PACKS=support,dev_tools` opts in to the others.

The `niato.run()` library API is unchanged — `createNiato({ packs })` still takes the explicit list as before. Plan 4 only changes the TUI's *default* packs.
...
1. **Worktree.** `/Users/aman-asmuei/Personals/niato/.worktrees/v1.0.0` on branch `release/v1.0.0` (already created from master HEAD `163f444`).
...
src/cli/tui/hooks/use-niato-session.ts Task 2  · use classifyError in catch block
src/cli/tui/index.tsx                    Task 3  · default packs = [genericPack]; NIATO_PACKS opt-in
...
**Files:**
- Modify: `src/cli/tui/hooks/use-niato-session.ts`

The catch block at `use-niato-session.ts:118-130` currently sets `errorMessage: err instanceof Error ? err.message : String(err)`. Replace with the classified message when available.

#### Step 1: Read the current catch block

```bash
sed -n '115,135p' src/cli/tui/hooks/use-niato-session.ts
```
...
```bash
git add src/cli/tui/hooks/use-niato-session.ts
...
The current TUI loads all three packs (`src/cli/tui/index.tsx:39`). Support's MCP is a stub — a stranger asking for a refund gets pretend data. Plan 4 makes the TUI Generic-only by default; Support and Dev Tools opt-in via `NIATO_PACKS=support,dev_tools`.

The `createNiato({ packs })` library API is untouched — embedders pass explicit packs as before.
...
Replace the existing `niatoFactory` block:
...
git commit -m "feat(tui): default to generic pack only; opt-in via NIATO_PACKS
...
packs via NIATO_PACKS=support,dev_tools.

The library API (createNiato({ packs })) is untouched; embedders
...
```markdown
**Default packs.** The TUI ships with the Generic pack only. To enable Support and/or Dev Tools (note: Support's MCP is a stub for demos), set `NIATO_PACKS=support,dev_tools` in your shell.
```
